...
class MultiHeadAttention(nn.Module):
    # changed default qkv_bias to True so checkpoint keys with biases match
    def __init__(self, d_in, d_out, context_length, dropout, num_heads, qkv_bias=True):
        ...
        self.out_proj = nn.Linear(d_out, d_out, bias=True)  # keep bias for out_proj
...
class TransformerBlock(nn.Module):
    def __init__(self, cfg):
        ...
        self.att = MultiHeadAttention(
            ...
            qkv_bias=cfg["qkv_bias"])  # cfg["qkv_bias"] will be passed but default is True
...
class GPTModel(nn.Module):
    ...
    def forward(self, in_idx):
        ...
        # clamp sequence length to maximum context length
        max_seq_len = self.pos_emb.num_embeddings
        if seq_len > max_seq_len:
            in_idx = in_idx[:, :max_seq_len]   # truncate extra tokens
            seq_len = max_seq_len
